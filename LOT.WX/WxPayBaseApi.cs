...
namespace OrderingSystem.WX
{
    public abstract class WxPayBaseApi
    {
        ...
        /// </summary>
        public static WxPayCenterConfig DefaultConfig
        {
            get { return WxPayConfig.config; }
            set { }
        }
        ...
        protected async Task<T> RestCommonAsync<T>(OsHttpRequest request) where T : WxPayBaseResp, new()
        {
            T t = default(T);
        ...
        /// <param name="client">自定义请求客户端，当前主要是因为标准库没有提供证书设置选项，所以通过上层运行时传入设置委托，在使用证书的子类中构造客户端传入 </param>
        /// <returns>实体类型</returns>
        protected T RestCommonAsyncExt<T>(OsHttpRequest request,
            Func<HttpResponseMessage, Task<T>> funcFormat = null, HttpClient client = null)
            where T : WxPayBaseResp, new()
        {
            T t = default(T);
            ...
            {
                //提交退款数据信息
                var resp = Post(request.CustomBody, request.AddressUrl, true, 6);
                //获取返回模型
                t = GetRespResult<T>(resp); 
            }
            catch (Exception ex)
            {
                t = new T() { Ret = (int)ResultTypes.InnerError, Message = ex.Message };
            ...
            try
            {
                ...
                * ************************************************************/
                ...
                if (isUseCert)
                {
                    ...
                    X509Certificate2 cert = new X509Certificate2(path + WxPayConfig.config.CertPath, WxPayConfig.config.CertPassword);
            ...
            Func<HttpResponseMessage, Task<T>> funcFormat = null, HttpClient client = null, Action<SortedDictionary<string, object>> dirformat = null) where T : WxPayBaseResp, new()
        {
            ...
            req.CustomBody = "<xml>" + xmlDirs.ProduceXml() + "</xml>";
            return await RestCommonAsync<T>(req, funcFormat, client);
        }

        /// <summary>
        /// (退款处理扩展) post 支付接口相关请求
        /// </summary>
        /// <typeparam name="T">返回参数类型</typeparam>
        /// <param name="addressUrl">接口地址</param>
        /// <param name="xmlDirs">请求参数的排序字典（不包括：appid,mch_id,sign。 会自动补充）</param>
        ...
        /// <param name="dirformat">生成签名后对字典发送前的操作，例如urlencode操作</param>
        /// <returns></returns>
        protected T PostApiAsyncExt<T>(string addressUrl, SortedDictionary<string, object> xmlDirs,
            Func<HttpResponseMessage, Task<T>> funcFormat = null, HttpClient client = null, Action<SortedDictionary<string, object>> dirformat = null) where T : WxPayBaseResp, new()
        {
            xmlDirs.Add("appid", ApiConfig.AppId);
            xmlDirs.Add("mch_id", ApiConfig.MchId);
            ...
            req.AddressUrl = addressUrl;
            req.CustomBody = "<xml>" + xmlDirs.ProduceXml() + "</xml>";
            return RestCommonAsyncExt<T>(req, funcFormat, client);
        }
