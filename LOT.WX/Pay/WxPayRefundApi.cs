...
namespace OrderingSystem.WX.Pay
{
    public class WxPayRefundApi:WxPayBaseApi
    {
        ...
        /// <param name="refundReq"></param>
        /// <returns></returns>
        public WxPayRefundResp RefundOrderAsyncExt(WxPayRefundReq refundReq)
        {
            var dics = refundReq.GetDics();
            var url = string.Concat(m_ApiUrl, "/secapi/pay/refund");
            var certClient = GetCertHttpClient();

            return PostApiAsyncExt<WxPayRefundResp>(url, dics, null, certClient);
        }
